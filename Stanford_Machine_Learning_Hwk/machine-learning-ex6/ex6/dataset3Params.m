...
for C_val = C_list
    for sigma_val = s_list
        ...
        % Train with a C and sigma value
        model= svmTrain(X, y, C_val, @(x1, x2) gaussianKernel(x1, x2, sigma_val));
        
        % Use the model to predict using the cross validation set
        prediction = svmPredict(model, Xval);
        
        % Compute the predicted - expected error
        err_val = mean(double(prediction ~= yval));
